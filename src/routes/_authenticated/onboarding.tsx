...
function Onboarding() {
  ...
  const [referralCode, setReferralCode] = useState(() =>
    typeof window === "undefined" ? "" : (sessionStorage.getItem("beauty:referralCode") ?? ""),
  );
  ...
  const { data: settings } = useQuery({
    ...
    queryFn: async () =>
      (await supabase.from("platform_settings").select("*").eq("id", true).maybeSingle())
        .data as any,
  });
  ...
  const canSubmit = useMemo(
    () =>
      ...
      !!nicheId,
    [companyName, ownerName, phone, nicheId],
  );
  ...
  const mutation = useMutation({
    ...
    onSuccess: async (res) => {
      sessionStorage.removeItem("beauty:referralCode");
      await refresh();
      toast.success("Empresa criada", {
        description: `Seu teste do plano Pro está ativo por ${res.trial_days} dias.`,
      });
      void navigate({ to: "/app", replace: true });
    },
    onError: (e) =>
      toast.error(e instanceof Error ? e.message : "Não foi possível criar sua empresa"),
  });
      ...
      <div className="mx-auto max-w-3xl space-y-6">
        <div className="text-center">
          ...
          <p className="mx-auto mt-3 max-w-xl text-sm text-muted-foreground">
            Comece sem falar com o suporte. Sua empresa recebe automaticamente acesso completo ao
            plano Pro durante o período de teste.
          </p>
        </div>
        ...
        <Card className="shadow-sm">
          <CardContent className="p-6 sm:p-8 space-y-5">
            <div className="rounded-xl border bg-primary/5 p-4 flex gap-3">
              ...
              <div>
                <p className="font-medium text-sm">Teste Pro por {trialDays} dias</p>
                <p className="text-xs text-muted-foreground mt-1">
                  Todos os recursos do plano Pro ficam liberados. Ao final do teste, o painel será
                  bloqueado caso não exista contratação ativa.
                </p>
              </div>
            </div>
              ...
              <div className="sm:col-span-2">
                ...
                {referralCode && (
                  <p className="mt-1 text-xs text-muted-foreground">
                    O benefício será liberado para quem indicou após o pagamento do seu primeiro
                    plano.
                  </p>
                )}
              </div>
            ...
            <div className="rounded-xl bg-muted/50 p-4 space-y-2 text-sm">
              <div className="flex gap-2">
                ...
                <span>Acesso completo ao plano Pro durante o teste</span>
              </div>
              <div className="flex gap-2">
                <CheckCircle2 className="h-4 w-4 text-primary mt-0.5" />
                <span>Nenhuma contratação é feita automaticamente</span>
              </div>
              <div className="flex gap-2">
                <CheckCircle2 className="h-4 w-4 text-primary mt-0.5" />
                <span>Você poderá escolher um plano antes do encerramento do período</span>
              </div>
            </div>
            ...
            >
              ...
              {mutation.isPending ? "Criando empresa..." : "Criar empresa e iniciar teste Pro"}
            </Button>
          </CardContent>
        </Card>
      </div>
