...
function Gate() {
  const { loading, isReseller, user, signOut } = useAuth();
  const nav = useNavigate();
  useEffect(() => {
    if (!loading && !isReseller) void nav({ to: "/home" as any, replace: true });
  }, [loading, isReseller, nav]);
  if (loading || !isReseller) return null;
  return (
    <div className="premium-shell min-h-screen">
      <header className="premium-header sticky top-0 z-20 flex h-16 items-center gap-3 border-b px-4 md:px-8">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">SeuAgendamento</p>
        </div>
