...
import { openWhatsAppLink, waLink } from "@/lib/format";
import {
...
function Reviews() {
  ...
  const filtered = useMemo(
    () =>
      data.filter((r) => {
        ...
        if (fService && !(r.service_names ?? "").toLowerCase().includes(fService.toLowerCase()))
          return false;
  ...
  const filteredAvg = filtered.length
    ? filtered.reduce((a, r) => a + r.rating, 0) / filtered.length
    : 0;
    ...
    <div className="space-y-6">
      <div className="flex items-end justify-between gap-4 flex-wrap">
        ...
        <div className="text-right">
          <div className="text-3xl font-semibold flex items-center gap-1 justify-end">
            {avg.toFixed(1)}
      ...
      <Tabs defaultValue="panel">
        ...
        <TabsContent value="panel" className="space-y-4 pt-4">
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <Kpi
              ...
              value={`${responseRate}%`}
              hint={`${answered} de ${sent} enviados`}
            />
            <Kpi
              icon={TrendingUp}
              label="Recomendariam"
              ...
              <CardContent className="space-y-2">
                {dist.map((d) => (
                  <div key={d.n} className="flex items-center gap-3">
                    <span className="w-10 text-sm flex items-center gap-1">
                      {d.n}
                      <Star className="h-3 w-3 fill-primary text-primary" />
                    </span>
                    <Progress
                      value={data.length ? (d.count / data.length) * 100 : 0}
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-base flex items-center gap-2">
                  <Users className="h-4 w-4" /> Nota por profissional
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-2">
                {byStaff.map((s) => (
                ...
                {byStaff.length === 0 && (
                  <p className="text-sm text-muted-foreground">Sem dados por profissional.</p>
                )}
              </CardContent>
            </Card>
          </div>
                  ...
                  {byMonth.map((m) => (
                    <div key={m.month} className="flex-1 flex flex-col items-center gap-1">
                      ...
                      <span className="text-[10px] text-muted-foreground">
                        {new Date(`${m.month}-01T00:00:00`).toLocaleDateString("pt-BR", {
                          month: "short",
                        })}
                      </span>
                    </div>
              ...
              <Card key={r.id} className={r.rating === 5 ? "border-primary/40" : undefined}>
                <CardContent className="p-4 space-y-2">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <div className="flex items-center gap-1 flex-wrap">
                        ...
                        {r.rating === 5 && (
                          <Badge className="ml-1 bg-emerald-500/15 text-emerald-700 dark:text-emerald-300 border-emerald-500/30">
                            Destaque
                          </Badge>
                        )}
                        ...
                        {r.rating <= NEGATIVE_ALERT_MAX_RATING && (
                          <Badge className="ml-1 bg-orange-500/15 text-orange-700 dark:text-orange-300 border-orange-500/30">
                            Atenção
                          </Badge>
                        )}
                      </div>
                      <div className="text-xs text-muted-foreground mt-1">
                        {r.customer_id
                          ? (custMap[r.customer_id]?.name ?? "Cliente")
                  ...
                  <div className="flex gap-3 text-xs text-muted-foreground">
                    ...
                    {r.would_return != null && (
                      <span>Voltaria: {r.would_return ? "sim" : "não"}</span>
                    )}
                    {r.would_recommend != null && (
                      <span>Recomenda: {r.would_recommend ? "sim" : "não"}</span>
                    )}
                  </div>
                </CardContent>
              </Card>
        ...
        <TabsContent value="link" className="pt-4">
          <PublicReviewLink
            companyId={companyId}
            companyName={activeCompany?.name ?? "nosso estabelecimento"}
          />
        </TabsContent>
  ...
  return (
    <div className="space-y-4 max-w-2xl">
      <Card>
        <CardHeader>
          <CardTitle className="text-base flex items-center gap-2">
            <Link2 className="h-4 w-4" /> Link exclusivo de avaliação
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <p className="text-sm text-muted-foreground">
            Endereço próprio da sua empresa. O cliente avalia sem login e tudo fica salvo aqui no
            sistema — sem Google Forms ou plataformas de terceiros.
          </p>
          ...
          <div className="flex gap-2 flex-wrap">
            <Button disabled={!url} onClick={() => openWhatsAppLink(waLink(null, waText))}>
            ...
            <Button
              ...
              onClick={() => {
                if (!confirm("Gerar um novo link? O endereço anterior deixará de funcionar."))
                  return;
                upsert.mutate(
                  { public_token: reviewToken(10) },
                  { onSuccess: () => toast.success("Novo link gerado") },
                );
              }}
            >
          ...
          <div className="flex items-center justify-between border-t pt-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Desative para bloquear novas avaliações por este link.
              </p>
            </div>
            <Switch
              checked={enabled}
              onCheckedChange={(v) => upsert.mutate({ public_link_enabled: v })}
            />
          </div>
        </CardContent>
      </Card>
  ...
  const save = useMutation({
    mutationFn: async () => {
      if (googleUrl && !/^https?:\/\//i.test(googleUrl))
        throw new Error("O link do Google deve começar com https://");
      const n = Number(days);
      if (!Number.isFinite(n) || n < 1 || n > 365)
        throw new Error("Validade deve ser entre 1 e 365 dias");
    ...
    <div className="space-y-4 max-w-2xl">
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          ...
          />
          <p className="text-xs text-muted-foreground">
            Clientes que avaliarem com 4 ou 5 estrelas serão convidados a repetir a avaliação no
            Google. Notas até {NEGATIVE_ALERT_MAX_RATING}★ ficam internas e geram alerta para a
            equipe.
          </p>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            Variáveis: {"{{NomeCliente}}"} {"{{Empresa}}"} {"{{Data}}"} {"{{Servico}}"}{" "}
            {"{{Funcionario}}"} {"{{LinkAvaliacao}}"}
          </p>
        </CardContent>
      </Card>
