...
function AppShell() {
        ...
        <Card className="max-w-md">
          <CardContent className="p-8 text-center">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              Conclua o cadastro da sua empresa para acessar o painel.
            </p>
          </CardContent>
        </Card>
      </div>
  ...
  const trialExpired =
    activeCompany.is_trial && !!activeCompany.trial_ends_at && activeCompany.trial_ends_at < today;
  const blockedStatus = ["suspended", "overdue", "trial_expired"].includes(
  ...
  if (blocked) {
    return (
      <div className="min-h-screen grid place-items-center bg-muted/20 p-6">
        <Card className="w-full max-w-lg shadow-sm">
          <CardContent className="p-8 sm:p-10 text-center">
            <div className="grid h-14 w-14 mx-auto place-items-center rounded-2xl bg-destructive/10 text-destructive">
            ...
            <p className="mt-3 text-sm leading-relaxed text-muted-foreground">
              O acesso ao painel foi bloqueado porque não há uma contratação ativa após o período de
              teste. Seus dados permanecem armazenados. Escolha um plano ou fale com o atendimento
              para reativar a empresa.
            </p>
            {activeCompany.trial_ends_at && (
              <p className="mt-4 text-xs text-muted-foreground">
                Teste encerrado em{" "}
                {new Date(`${activeCompany.trial_ends_at}T12:00:00`).toLocaleDateString("pt-BR")}.
              </p>
            )}
          </CardContent>
        </Card>
...
function PermissionGate() {
  ...
  const { isSuperAdmin } = useAuth();
  const { can, hasFeature, isProfessional, loading } = usePermissions();
  ...
  const proPlanDenied =
    path.startsWith("/app/marketing") && activeCompany?.plan_code?.trim().toLowerCase() !== "pro";
  const professionalAllowed =
    path === "/app" ||
    path === "/app/" ||
    path.startsWith("/app/professional") ||
    path.startsWith("/app/agenda") ||
    path.startsWith("/app/blocks") ||
    path.startsWith("/app/commissions") ||
    path.startsWith("/app/help");
  const professionalDenied = isProfessional && !professionalAllowed;

  if (permissionDenied || planDenied || proPlanDenied || professionalDenied) {
    const deniedByPlan = planDenied || proPlanDenied;
    return (
      <Card className="max-w-lg mx-auto">
        <CardContent className="p-8 text-center">
          <h1 className="text-lg font-semibold">
            {deniedByPlan ? "Recurso exclusivo do plano Pro" : "Acesso não autorizado"}
          </h1>
          <p className="mt-2 text-sm text-muted-foreground">
            {deniedByPlan
              ? "O estúdio de Marketing e publicidade com IA está disponível para empresas do plano Pro. Fale com o administrador para alterar o plano da empresa."
