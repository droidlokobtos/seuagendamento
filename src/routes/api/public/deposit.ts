    ...
    handlers: {
      POST: async ({ request }) => {
        ...
        const { data: appt } = await supabaseAdmin
          .from("appointments")
          .select("id,company_id,customer_id,deposit_required_cents,total_cents,discount_cents,surcharge_cents,paid_cents")
          .eq("id", appointment_id).maybeSingle();
        if (!appt) return Response.json({ error: "Agendamento não encontrado" }, { status: 404 });

        // Autorização simples: o telefone informado precisa bater com o do cliente
        const { data: cust } = await supabaseAdmin
          .from("customers").select("id,phone").eq("id", appt.customer_id ?? "").maybeSingle();
        if (!cust || onlyDigits(cust.phone ?? "").slice(-8) !== onlyDigits(phone).slice(-8)) {
          return Response.json({ error: "Não foi possível validar seu agendamento" }, { status: 403 });
        }
        ...
        if (file_base64) {
          ...
          if (!allowed.includes(mime)) return Response.json({ error: "Envie imagem ou PDF" }, { status: 400 });
          ...
          const { error: upErr } = await supabaseAdmin.storage
            .from("company-assets").upload(path, bytes, { contentType: mime, upsert: false });
          if (upErr) return Response.json({ error: "Falha ao enviar comprovante" }, { status: 500 });
          proofUrl = path;
        }
