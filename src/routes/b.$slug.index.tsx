...
function BookingPage() {
  ...
  const canContinue = (() => {
    ...
    if (step === 4) return !staffLoading && staffList.length > 0;
      ...
      <div className="min-h-screen bg-background">
        ...
        <div className="max-w-lg mx-auto p-6 space-y-4">
          <Card><CardContent className="p-8 text-center space-y-4">
            <div className="mx-auto grid h-14 w-14 place-items-center rounded-full" style={{ background: accent }}><Check className="h-7 w-7 text-white" /></div>
            <h2 className="text-2xl font-semibold">{depositCents > 0 ? "Agendamento registrado!" : "Agendamento confirmado!"}</h2>
            <p className="text-muted-foreground">{dateLabel} às {timeLabel}</p>
            ...
            <div className="flex flex-col gap-2 items-center pt-2">
              {session ? <Link to="/b/$slug/minha-conta" params={{ slug: company.slug }}><Button style={{ background: primary }}>Ver meus agendamentos</Button></Link> : <Link to="/b/$slug/entrar" params={{ slug: company.slug }}><Button style={{ background: primary }}>Criar conta e acompanhar</Button></Link>}
              {company.whatsapp && (() => { const d = company.whatsapp.replace(/\D/g, ""); const p = d.startsWith("55") ? d : `55${d}`; return <a href={`https://api.whatsapp.com/send?phone=${p}&text=${waMsg}`} target="_blank" rel="noreferrer"><Button style={{ background: "#25D366", color: "white" }}><Phone className="h-4 w-4 mr-2" /> Enviar confirmação no WhatsApp</Button></a>; })()}
            </div>
          </CardContent></Card>
  ...
  return (
    <div className="min-h-screen bg-background pb-20">
      ...
      <div className="max-w-lg mx-auto p-4 md:p-6 space-y-4">
        ...
        {step === 1 && <Card><CardContent className="p-4 space-y-3"><h2 className="font-semibold text-lg">Escolha os serviços</h2>{!services.length && <p className="text-sm text-muted-foreground">Nenhum serviço disponível.</p>}<div className="space-y-2">{services.map((s) => { const active = selected.some((x) => x.id === s.id); return <button key={s.id} onClick={() => toggleService(s)} className={`w-full text-left rounded-xl border overflow-hidden transition ${active ? "border-primary bg-primary/5" : "border-border hover:bg-muted/50"}`}>{s.photo_url && <div className="h-32 w-full bg-muted overflow-hidden"><img src={s.photo_url} alt={s.name} className="h-full w-full object-cover" style={svcFrame(s.photo_position)} /></div>}<div className="p-3 flex items-center justify-between gap-2"><div className="min-w-0"><p className="font-medium truncate">{s.name}</p><p className="text-xs text-muted-foreground">{s.duration_min} min{s.category ? ` · ${s.category}` : ""}</p></div><div className="text-right"><p className="font-semibold">{brl(s.price_cents / 100)}</p>{active && <Badge variant="secondary" className="mt-1"><Check className="h-3 w-3 mr-1" /> Selecionado</Badge>}</div></div></button>; })}</div></CardContent></Card>}
        {step === 2 && <Card><CardContent className="p-4 space-y-4"><h2 className="font-semibold text-lg">Escolha a data</h2><div><Label className="text-xs mb-2 flex items-center gap-1"><Calendar className="h-3 w-3" /> Data</Label><div className="flex gap-2 overflow-x-auto pb-2 -mx-1 px-1">{dateOptions.map((d) => <button key={d.iso} disabled={d.disabled} onClick={() => { setDateStr(d.iso); setTimeStr(""); setStaff(null); }} className={`shrink-0 rounded-xl border px-3 py-2 min-w-16 text-center transition ${d.disabled ? "opacity-40 cursor-not-allowed" : dateStr === d.iso ? "border-primary bg-primary text-primary-foreground" : "border-border hover:bg-muted/50"}`}><p className="text-[10px] uppercase tracking-wider">{WEEKDAYS[d.wd]}</p><p className="font-semibold text-sm">{d.label}</p></button>)}</div>{dateOptions.every((d) => d.disabled) && <p className="text-xs text-muted-foreground text-center py-2">Sem datas disponíveis. Entre em contato pelo WhatsApp.</p>}</div></CardContent></Card>}
        {step === 3 && <Card><CardContent className="p-4 space-y-4"><h2 className="font-semibold text-lg">Escolha o horário</h2><div><Label className="text-xs mb-2 flex items-center gap-1"><Clock className="h-3 w-3" /> Horário</Label>{!slots.length ? <p className="text-sm text-muted-foreground text-center py-6">Sem horários disponíveis nesta data.</p> : <div className="grid grid-cols-4 gap-2">{slots.map((t) => <button key={t} onClick={() => { setTimeStr(t); setStaff(null); }} className={`rounded-lg border py-2 text-sm transition ${timeStr === t ? "border-primary bg-primary text-primary-foreground" : "border-border hover:bg-muted/50"}`}>{t}</button>)}</div>}</div></CardContent></Card>}
        {step === 4 && showStaffStep && <Card><CardContent className="p-4 space-y-3"><h2 className="font-semibold text-lg">Profissionais disponíveis</h2><p className="text-xs text-muted-foreground">Somente quem atende os serviços escolhidos e está livre em {dateStr.split("-").reverse().join("/")} às {timeStr}.</p>{staffLoading ? <p className="text-sm text-muted-foreground text-center py-6">Buscando profissionais…</p> : !staffList.length ? <div className="rounded-xl border border-dashed p-4 text-center space-y-2"><p className="text-sm font-medium">{staffReason === "no_link" ? "Nenhum profissional atende essa combinação de serviços." : staffReason === "no_staff" ? "Nenhum profissional cadastrado para atender." : "Nenhum profissional disponível neste horário."}</p><p className="text-xs text-muted-foreground">Volte e escolha outro horário ou outra data.</p><Button variant="outline" size="sm" onClick={() => setStep(3)}>Escolher outro horário</Button></div> : <>{staffList.length > 1 && <button onClick={() => setStaff(null)} className={`w-full text-left rounded-xl border p-3 ${!staff ? "border-primary bg-primary/5" : "border-border"}`}><p className="font-medium">Qualquer profissional</p><p className="text-xs text-muted-foreground">A empresa define quem atende</p></button>}{staffList.map((s) => <button key={s.id} onClick={() => setStaff(s)} className={`w-full text-left rounded-xl border p-3 flex items-center gap-3 ${staff?.id === s.id ? "border-primary bg-primary/5" : "border-border hover:bg-muted/50"}`}>{s.photo_url ? <img src={s.photo_url} alt="" className="h-10 w-10 rounded-full object-cover" /> : <div className="h-10 w-10 rounded-full grid place-items-center text-white text-sm font-semibold" style={{ background: s.color ?? accent }}>{s.name.charAt(0)}</div>}<div className="min-w-0"><p className="font-medium truncate">{s.name}</p>{s.role_title && <p className="text-xs text-muted-foreground truncate">{s.role_title}</p>}</div></button>)}</>}</CardContent></Card>}
        {step === 5 && <Card><CardContent className="p-4 space-y-3"><h2 className="font-semibold text-lg">Seus dados</h2><div><Label>Nome</Label><Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /></div><div><Label>WhatsApp</Label><Input value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} placeholder="(11) 99999-9999" /></div><div><Label>E-mail (opcional)</Label><Input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} /></div><div><Label>Observações (opcional)</Label><Textarea value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} /></div><div><Label>Cupom de desconto (opcional)</Label><div className="flex gap-2"><Input value={couponCode} onChange={(e) => { setCouponCode(e.target.value.toUpperCase()); setCoupon(null); }} placeholder="CODIGO" /><Button type="button" variant="outline" disabled={validating || !couponCode.trim()} onClick={applyCoupon}>{validating ? "…" : coupon ? "OK" : "Aplicar"}</Button></div>{coupon && <p className="text-xs text-green-700 mt-1">Desconto de {brl(coupon.discount_cents / 100)} aplicado.</p>}</div></CardContent></Card>}
...
function Hero({ company, primary, accent, slug, loggedIn }: { company: any; primary: string; accent: string; slug: string; loggedIn: boolean }) {
  const theme = portalTheme(company);
  return <div className="relative" style={heroBackground(theme)}>{theme.bgUrl && <img src={theme.bgUrl} className="absolute inset-0 h-full w-full object-cover" style={{ opacity: heroImageOpacity(theme) }} alt="" />}<div className={`relative max-w-lg mx-auto px-6 py-10 ${heroTextClass(theme)}`}><div className="flex items-start gap-3">{company.logo_url ? <img src={company.logo_url} className="h-14 w-14 rounded-2xl object-cover ring-2 ring-white/40" alt="" /> : <div className="h-14 w-14 rounded-2xl grid place-items-center bg-white/15 ring-2 ring-white/40"><Sparkles className="h-7 w-7" /></div>}<div className="flex-1 min-w-0"><h1 className="text-2xl font-semibold leading-tight truncate">{company.name}</h1>{theme.slogan && <p className="text-xs opacity-85 truncate">{theme.slogan}</p>}{company.address && <p className="text-xs opacity-80 flex items-center gap-1 mt-0.5"><MapPin className="h-3 w-3" /> {company.address}</p>}</div><Link to={loggedIn ? "/b/$slug/minha-conta" : "/b/$slug/entrar"} params={{ slug }} className="shrink-0 rounded-full bg-white/15 hover:bg-white/25 backdrop-blur px-3 py-1.5 text-xs font-medium ring-1 ring-white/30 flex items-center gap-1.5"><User className="h-3.5 w-3.5" /> {loggedIn ? "Minha conta" : "Entrar"}</Link></div></div></div>;
}
...
function ReviewsSection({ companyId, accent }: { companyId: string; accent: string }) {
  ...
  return <Card><CardContent className="p-4 space-y-3"><div className="flex items-center justify-between"><h2 className="font-semibold">Avaliações</h2><div className="flex items-center gap-1 text-sm"><Star className="h-4 w-4 fill-current" style={{ color: accent }} /><span className="font-semibold">{avg.toFixed(1)}</span><span className="text-xs text-muted-foreground">({reviews.length})</span></div></div><div className="space-y-2">{reviews.slice(0, 3).map((r) => <div key={r.id} className="rounded-lg border p-3 text-sm"><div className="flex items-center gap-1 mb-1">{Array.from({ length: 5 }).map((_, i) => <Star key={i} className={`h-3.5 w-3.5 ${i < r.rating ? "fill-current" : ""}`} style={{ color: i < r.rating ? accent : "#ccc" }} />)}<span className="text-xs text-muted-foreground ml-1">{r.customers?.name ?? r.customer_name ?? "Cliente"}</span></div>{r.comment && <p className="text-xs text-muted-foreground">{r.comment}</p>}</div>)}</div></CardContent></Card>;
}
