...
const Input = z.object({
  ...
  referral_code: z
    ...
    .trim()
    .toUpperCase()
    .regex(/^[A-Z0-9]{8}$/, "Código de indicação inválido")
    .optional(),
});
...
export const createSelfServiceTrialCompany = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    if (data.referral_code) {
      const { data: referralCode } = await (supabaseAdmin.from as any)("company_referral_codes")
        .select("company_id")
        .eq("code", data.referral_code)
        .maybeSingle();
      if (!referralCode?.company_id) throw new Error("Código de indicação inválido.");
      referrerCompanyId = String(referralCode.company_id);
    }

    const trialDays = Math.max(
      1,
      Math.min(365, Number((settings as any)?.default_trial_days ?? 15)),
    );
    ...
    await supabaseAdmin
      .from("profiles")
      .upsert({ id: userId, full_name: data.owner_name, phone: data.phone } as any, {
        onConflict: "id",
      });
    await supabaseAdmin
      .from("company_users")
    ...
    await supabaseAdmin
      .from("user_roles")
      .upsert({ user_id: userId, role: "company_admin" } as any, { onConflict: "user_id,role" });
    if (data.referral_code && referrerCompanyId) {
      const { error: referralError } = await (supabaseAdmin.from as any)(
        "company_referrals",
      ).insert({
      ...
      if (referralError) throw new Error(referralError.message || "Código de indicação inválido.");
    }
    await supabaseAdmin
      ...
      .insert({
        user_id: userId,
        email,
        event: "self_service_trial_created",
        metadata: {
          company_id: company.id,
          plan_code: "pro",
          trial_days: trialDays,
