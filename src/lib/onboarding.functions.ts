...
export const createSelfServiceTrialCompany = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const [{ data: authUser }, { data: settings }, { data: proPlan }] = await Promise.all([
      ...
      supabaseAdmin.from("platform_settings").select("default_trial_days").eq("id", true).maybeSingle(),
    ...
    await supabaseAdmin.from("admin_access_logs").insert({
      ...
      metadata: { company_id: company.id, plan_code: "pro", trial_days: trialDays, trial_ends_at: company.trial_ends_at },
    } as any);
