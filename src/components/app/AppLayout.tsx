...
const GROUPS: NavGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      {
        to: "/app/procedures",
        label: "Calculadora de Procedimentos",
        icon: Calculator,
        perm: "servicos",
      },
    ],
  },
  {
    ...
    items: [
      ...
      {
        to: "/app/expenses",
        label: "Despesas e contas a pagar",
        icon: ReceiptText,
        perm: "financeiro",
      },
  ...
  },
  {
    ...
    icon: Package,
    items: [
      { to: "/app/products", label: "Produtos e movimentações", icon: Package, perm: "estoque" },
    ],
  },
  {
    id: "equipe",
    ...
    <div className="flex items-center gap-3 min-w-0">
      {activeCompany?.logo_url ? (
        <img
          ...
          className="h-10 w-10 shrink-0 rounded-xl object-cover ring-1 ring-border shadow-sm"
          alt=""
        />
      ) : (
        ...
        return (
          <div key={group.id} className="rounded-xl">
            <button
              ...
              className={`flex w-full items-center gap-2.5 rounded-lg px-3 py-2 text-[11px] font-semibold uppercase tracking-[0.12em] transition-all ${groupActive ? "text-foreground bg-muted/60" : "text-muted-foreground hover:bg-muted/40 hover:text-foreground"}`}
            >
        ...
        <aside className="hidden md:flex md:w-72 shrink-0 border-r border-border/70 bg-card/90 backdrop-blur-xl flex-col shadow-[8px_0_30px_-24px_rgba(0,0,0,0.25)]">
          ...
          <div className="border-t border-border/70 p-4 bg-muted/20">
            <div className="mb-3 px-1">
              ...
              <p className="mt-0.5 text-[10px] text-muted-foreground truncate">{user?.email}</p>
            </div>
            <Button
              variant="ghost"
              size="sm"
          ...
          <header className="h-[72px] border-b border-border/70 bg-card/80 backdrop-blur-xl flex items-center gap-3 px-4 md:px-7 sticky top-0 z-30 shadow-[0_8px_30px_-26px_rgba(0,0,0,0.35)]">
            <Sheet open={open} onOpenChange={setOpen}>
              ...
              >
                ...
                <div className="shrink-0 border-t border-border/60 p-4">
                  <p className="mb-2 px-1 text-[10px] text-muted-foreground truncate">
                    {user?.email}
                  </p>
                  <Button
                    variant="ghost"
            ...
            {companies.length > 1 ? (
              <Select value={activeCompany?.id} onValueChange={setActiveCompanyId}>
                <SelectTrigger className="w-[180px] md:w-[230px] h-10 rounded-xl bg-background/70 border-border/80">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {companies.map((c) => (
            ...
            <div className="ml-auto flex items-center gap-2">
              <div className="hidden lg:flex items-center gap-2">
                {quickActions.map((a) => (
                  <Button
                    ...
                    variant="outline"
                    size="sm"
                    className="rounded-xl bg-background/60 shadow-sm"
                  >
                    <Link to={a.to}>
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button size="sm" className="lg:hidden rounded-xl">
      ...
      <nav className="md:hidden fixed bottom-0 inset-x-0 z-40 border-t border-border/70 bg-card/95 backdrop-blur-xl shadow-[0_-10px_30px_-25px_rgba(0,0,0,0.3)]">
        <div className="flex justify-around">
          {bottomNav.map((item) => {
            ...
            return (
              <Link
                key={item.to}
                to={item.to}
                className={`flex min-w-16 flex-col items-center gap-1 py-2.5 text-[10px] font-medium ${active ? "text-primary" : "text-muted-foreground"}`}
              >
          ...
          <button
            ...
            className="flex min-w-16 flex-col items-center gap-1 py-2.5 text-[10px] font-medium text-muted-foreground"
          >
