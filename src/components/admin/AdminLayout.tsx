...
import { LayoutDashboard, Building2, Tag, CreditCard, Settings, LogOut, Sparkles, Menu, ShieldCheck, Layers3 } from "lucide-react";
    ...
    <div className="min-h-screen bg-background flex">
      <aside className="hidden md:flex md:w-64 shrink-0 border-r border-border/60 bg-card/50 flex-col">
        <div className="h-16 flex items-center gap-2 px-4 border-b border-border/60">
          ...
          <div className="min-w-0"><p className="text-sm font-semibold leading-tight truncate">BeautySaaS</p><p className="text-[10px] uppercase tracking-wider text-muted-foreground">Admin Master</p></div>
        </div>
        <div className="flex-1 overflow-y-auto"><NavList /></div>
        <div className="border-t border-border/60 p-3">
          <div className="mb-2 px-1"><p className="text-xs font-medium truncate">{user?.user_metadata?.full_name ?? user?.email}</p><p className="text-[10px] text-muted-foreground truncate">{user?.email}</p></div>
        ...
        <header className="h-16 border-b border-border/60 bg-card/40 backdrop-blur flex items-center gap-3 px-4 md:px-6 sticky top-0 z-30">
          <Sheet open={open} onOpenChange={setOpen}>
            ...
            <SheetContent side="left" className="p-0 w-64 flex flex-col h-[100dvh] max-h-[100dvh] overflow-hidden">
              <div className="h-16 shrink-0 flex items-center gap-2 px-4 border-b"><div className="grid h-8 w-8 place-items-center rounded-lg bg-primary text-primary-foreground"><Sparkles className="h-4 w-4" /></div><span className="text-sm font-semibold">BeautySaaS</span></div>
              <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain" style={{ WebkitOverflowScrolling: "touch" }}><NavList /></div>
            </SheetContent>
          </Sheet>
          <div className="min-w-0"><h1 className="text-sm font-medium text-muted-foreground">Admin Master</h1></div>
        </header>
